...
bands.get("/:name", async (req, res) => {
  try {
    const foundBand = await Band.findOne({
      ...
      include: [
        {
          model: MeetGreet, //   this includes any meetGreet data associated with named band
          as: "meet_greets",
          include: {
            model: Event, //  This is a nested includes events associated with the meetGreet data
            as: "event",
            where: {
              name: {
                [Op.like]: `%${req.query.event ? req.query.event : ""}%`, //  This filters the event by name in the query params
              },
            },
          },
        },
        {
          model: SetTime, //  This includes set times
          as: "set_times",
          include: {
            model: Event,
            as: "event",
